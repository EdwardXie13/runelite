...
public class U_OgreBow extends ChargedItem {
    public U_OgreBow(
            final Client client,
        ...
        super(ChargesImprovedConfig.ogre_bow, ItemID.OGRE_BOW, client, client_thread, configs, items, infoboxes, chat_messages, notifier, config, store, gson);

        this.items = new TriggerItem[]{
                new TriggerItem(ItemID.OGRE_BOW),
        };

        this.triggers = new TriggerBase[] {
                // Check.
                new OnChatMessage("You've scratched up a total of (?<charges>.+) chompy bird kills so far!").setDynamicallyCharges().onItemClick(),

                // Add kill.
                new OnChatMessage("You scratch a notch on your bow for the chompy bird kill").increaseCharges(1),
        };
    }
}
